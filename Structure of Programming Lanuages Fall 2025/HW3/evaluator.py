...
def evaluate(ast, environment, watched_identifier=None):
    if ast["tag"] == "number":
    ...
    if ast["tag"] == "list":
        items = []
        for item in ast["items"]:
            result, item_status = evaluate(item, environment, watched_identifier)
    ...
    if ast["tag"] == "object":
        object = {}
        for item in ast["items"]:
            key, key_status = evaluate(item["key"], environment, watched_identifier)
            if key_status == "exit": return key, "exit"
            assert type(key) is str, "Object key must be a string"
            value, value_status = evaluate(item["value"], environment, watched_identifier)
    ...
    if ast["tag"] == "identifier":
        ...
        if "$parent" in environment:
            return evaluate(ast, environment["$parent"], watched_identifier)
    ...
    if ast["tag"] == "+":
        left_value, l_status = evaluate(ast["left"], environment, watched_identifier)
        if l_status == "exit": return left_value, "exit"
        right_value, r_status = evaluate(ast["right"], environment, watched_identifier)
    ...
    if ast["tag"] == "negate":
        value, status = evaluate(ast["value"], environment, watched_identifier)
    ...
    if ast["tag"] == "assert":
        if ast["condition"]:
            condition_value, cond_status = evaluate(ast["condition"], environment, watched_identifier)
            ...
            if not is_truthy(condition_value):
                ...
                if "explanation" in ast and ast["explanation"]:
                    explanation_val, expl_status = evaluate(ast["explanation"], environment, watched_identifier)
    ...
    if ast["tag"] == "if":
        ...
        if is_truthy(condition_value):
            val, status = evaluate(ast["then"], environment, watched_identifier)
            if status: # Propagate "return", "exit", "break", "continue"
                return val, status
        else:
            if "else" in ast:
                val, status = evaluate(ast["else"], environment, watched_identifier)
                if status: # Propagate "return", "exit", "break", "continue"
                    return val, status
    ...
    if ast["tag"] == "while":
        ...
        while is_truthy(condition_value):
            val, body_status = evaluate(ast["do"], environment, watched_identifier)
    ...
    if ast["tag"] == "statement_list":
        ...
        for statement in ast["statements"]:
            last_value, status = evaluate(statement, environment, watched_identifier)
    ...
    if ast["tag"] == "program":
        last_value = None
        for statement in ast["statements"]:
            val, status = evaluate(statement, environment, watched_identifier)
            if status:
                if status == "return":
    ...
    if ast["tag"] == "call":
        function, func_status = evaluate(ast["function"], environment, watched_identifier)
        if func_status == "exit": return function, "exit"
        argument_values = []
        for arg in ast["arguments"]:
            arg_val, arg_status = evaluate(arg, environment, watched_identifier)
        ...
        val, status = evaluate(function["body"], local_environment, watched_identifier)
    ...
    if ast["tag"] == "complex":
        base, base_status = evaluate(ast["base"], environment, watched_identifier)
        if base_status == "exit": return base, "exit"
        index, index_status = evaluate(ast["index"], environment, watched_identifier)
    ...
    if ast["tag"] == "assign":
        ...
        if target["tag"] == "identifier":
            ...
            base, base_status = evaluate(target["base"], environment, watched_identifier)
            if base_status == "exit": return base, "exit"
        ...
        value, value_status = evaluate(ast["value"], environment, watched_identifier)
        if value_status == "exit": return value, "exit"

        if target["tag"] == "identifier":
            name = target["value"]
            if watched_identifier is not None and name == watched_identifier:
                # Attempt to get line and column information from the AST node
                line = target.get("line", "??")
                col = target.get("column", "??")
                position = target.get("position", "??")
                
                location = f"L{line}:C{col}" if line != "??" and col != "??" else f"Pos {position}"
                
                print(f"*** WATCH: '{name}' changed at {location} to {value} ***")
    ...
    if ast["tag"] == "return":
        if "value" in ast and ast["value"] is not None: # Checks if 'return' has an expression
            evaluated_value, expression_status = evaluate(ast["value"], environment, watched_identifier)
    ...
    if ast["tag"] == "exit":
        ...
        if "value" in ast and ast["value"] is not None:
            exit_code_val, status = evaluate(ast["value"], environment, watched_identifier)
    ...
    if ast["tag"] == "import":
        filename_val, status = evaluate(ast["value"], environment, watched_identifier)
        ...
        try:
            ...
            return evaluate(imported_ast, environment, watched_identifier) # Propagates value and status from imported code
